 ...
 * sf37-server still owns the automatic byte-prefix cache policy built on top of
 * this file; sf37-agent uses only the same durable format for explicit sessions,
 * with its own policy in sf37_agent.c.  Protocol-specific extras, such as the
 ...
 * sf37_session internals become unsafe to restore across runtime changes. */
#define KV_CACHE_PAYLOAD_ABI 2u
#define SF37_KVSTORE_MODEL_ID 37u
#define SF37_KVSTORE_QUANT_BITS 3u
...
static const char *kv_log_name(const sf37_kvstore *kc) {
    return kc && kc->log_name ? kc->log_name : "sf37";
}

static void kv_logf(sf37_kvstore *kc, sf37_kvstore_log_type type,
                    const char *fmt, ...) {
    if (!kc || !kc->log) return;
    va_list ap;
    va_start(ap, fmt);
...
uint8_t sf37_kvstore_reason_code(const char *reason) {
    if (!reason) return SF37_KVSTORE_REASON_UNKNOWN;
    if (!strcmp(reason, "cold")) return SF37_KVSTORE_REASON_COLD;
    if (!strcmp(reason, "continued")) return SF37_KVSTORE_REASON_CONTINUED;
    if (!strcmp(reason, "evict")) return SF37_KVSTORE_REASON_EVICT;
    if (!strcmp(reason, "shutdown")) return SF37_KVSTORE_REASON_SHUTDOWN;
    if (!strcmp(reason, "agent-system")) return SF37_KVSTORE_REASON_AGENT_SYSTEM;
    if (!strcmp(reason, "agent-session")) return SF37_KVSTORE_REASON_AGENT_SESSION;
    return SF37_KVSTORE_REASON_UNKNOWN;
}

const char *sf37_kvstore_key_kind(uint8_t ext_flags) {
    if (ext_flags & SF37_KVSTORE_EXT_RESPONSES_VISIBLE) return "responses-visible";
    if (ext_flags & SF37_KVSTORE_EXT_THINKING_VISIBLE) return "thinking-visible";
    if (ext_flags & SF37_KVSTORE_EXT_IMAGE_KEY) return "image-key";
    return "token-text";
}
...
char *sf37_kvstore_path_for_sha(sf37_kvstore *kc, const char sha[41]) {
    ...
    memcpy(name, sha, 40);
    memcpy(name + 40, ".kv", 4);
    return sf37_kvstore_path_join(kc->dir, name);
}
...
void sf37_kvstore_clear(sf37_kvstore *kc) {
    for (int i = 0; i < kc->len; i++) sf37_kvstore_entry_free(&kc->entry[i]);
    free(kc->entry);
    kc->entry = NULL;
    kc->len = 0;
    kc->cap = 0;
...
static void kv_cache_push(sf37_kvstore *kc, sf37_kvstore_entry e) {
    if (kc->len == kc->cap) {
...
void sf37_kvstore_fill_header(uint8_t h[SF37_KVSTORE_FIXED_HEADER],
    ...
    memset(h, 0, SF37_KVSTORE_FIXED_HEADER);
    h[0] = KV_CACHE_MAGIC0;
    h[1] = KV_CACHE_MAGIC1;
    h[2] = KV_CACHE_MAGIC2;
    ...
    h[6] = ext_flags;
    h[7] = model_id;
    sf37_kvstore_le_put32(h + 8, tokens);
    sf37_kvstore_le_put32(h + 12, hits);
    sf37_kvstore_le_put32(h + 16, ctx_size);
...
bool sf37_kvstore_read_header(FILE *fp, sf37_kvstore_entry *e,
                             uint32_t *text_bytes) {
    uint8_t h[SF37_KVSTORE_FIXED_HEADER];
    ...
    e->quant_bits = h[4];
    e->reason = h[5] <= SF37_KVSTORE_REASON_AGENT_SESSION ? h[5] :
                SF37_KVSTORE_REASON_UNKNOWN;
    e->ext_flags = h[6];
    e->model_id = h[7];
    e->tokens = sf37_kvstore_le_get32(h + 8);
    e->hits = sf37_kvstore_le_get32(h + 12);
    e->ctx_size = sf37_kvstore_le_get32(h + 16);
    ...
    *text_bytes = sf37_kvstore_le_get32(tb);
    e->text_bytes = *text_bytes;
    return e->tokens != 0 && e->quant_bits == SF37_KVSTORE_QUANT_BITS;
}
...
bool sf37_kvstore_read_entry_file(const char *path, const char sha[41],
    ...
    struct stat st;
    if (stat(path, &st) != 0 ||
        st.st_size < (off_t)(SF37_KVSTORE_FIXED_HEADER + 4))
        return false;
    FILE *fp = fopen(path, "rb");
    if (!fp) return false;
    sf37_kvstore_entry e = {0};
    uint32_t text_bytes = 0;
    bool ok = sf37_kvstore_read_header(fp, &e, &text_bytes);
    fclose(fp);
    if (!ok) return false;
    const uint64_t fixed = SF37_KVSTORE_FIXED_HEADER + 4ull;
...
static void kv_cache_refresh(sf37_kvstore *kc) {
    if (!kc->enabled) return;
    sf37_kvstore_clear(kc);
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        char *path = sf37_kvstore_path_join(kc->dir, de->d_name);
        sf37_kvstore_entry e = {0};
        if (sf37_kvstore_read_entry_file(path, sha, &e)) kv_cache_push(kc, e);
        free(path);
    }
    closedir(d);
}
...
bool sf37_kvstore_touch_file(const char *path, uint32_t hits) {
    ...
    if (ok) {
        ...
        sf37_kvstore_fill_header(h, e.model_id, e.quant_bits, e.reason, e.ext_flags,
...
static bool kv_cache_incoming_supersedes_continued(
    ...
    char prefix_sha[41];
    sf37_kvstore_sha1_bytes_hex(incoming->text, (size_t)e->text_bytes,
                               prefix_sha);
    return !strcmp(prefix_sha, e->sha);
}

static bool kv_cache_reason_is_anchor(uint8_t reason) {
    return reason == SF37_KVSTORE_REASON_COLD ||
           reason == SF37_KVSTORE_REASON_EVICT ||
           reason == SF37_KVSTORE_REASON_SHUTDOWN;
}

double sf37_kvstore_entry_eviction_score(
        const sf37_kvstore_entry *e,
    ...
    if (used_at == 0) {
        ...
        effective_hits *= exp2(-elapsed / (double)SF37_KVSTORE_HIT_HALF_LIFE_SECONDS);
...
void sf37_kvstore_evict(sf37_kvstore *kc, const sf37_tokens *live,
    ...
    while (total > target && kc->len > 0) {
        int victim = 0;
        double victim_score =
            sf37_kvstore_entry_eviction_score(&kc->entry[0], live, now,
                                             incoming);
        for (int i = 1; i < kc->len; i++) {
            double score =
                sf37_kvstore_entry_eviction_score(&kc->entry[i], live, now,
                                                 incoming);
        ...
        sf37_kvstore_entry e = kc->entry[victim];
        if (unlink(e.path) == 0) {
            kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
...
bool sf37_kvstore_open(sf37_kvstore *kc, const char *dir, uint64_t budget_mb,
    ...
    kc->log_ud = log_ud;
    if (!kv_mkdir_p(dir)) {
        kv_logf(kc, SF37_KVSTORE_LOG_DEFAULT,
    ...
    kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
            ...
            (unsigned long long)SF37_KVSTORE_HIT_HALF_LIFE_SECONDS);
    return true;
}
...
char *sf37_kvstore_render_tokens_text(sf37_engine *engine,
    ...
    for (int i = 0; i < tokens->len; i++) {
        size_t len = 0;
        char *piece = sf37_token_text(engine, tokens->v[i], &len);
...
void sf37_kvstore_tokens_copy_prefix(sf37_tokens *dst, const sf37_tokens *src, int n) {
    ...
    for (int i = 0; i < n; i++) sf37_tokens_push(dst, src->v[i]);
}

static void tokens_append(sf37_tokens *dst, const sf37_tokens *src) {
    if (!dst || !src) return;
...
void sf37_kvstore_build_prompt_from_exact_prefix_and_text_suffix(
        ...
        sf37_tokens *out) {
    sf37_tokens_copy(out, exact_prefix);

    sf37_tokens suffix = {0};
    /* The suffix may start with DS4 chat markers such as <｜User｜> or
     * </think>, so use the rendered-chat tokenizer, not plain text BPE. */
    sf37_tokenize_rendered_chat(engine, suffix_text ? suffix_text : "", &suffix);
    tokens_append(out, &suffix);
    sf37_tokens_free(&suffix);
}
...
static int kv_cache_continued_step(const sf37_kvstore *kc) {
...
int sf37_kvstore_suppress_continued_store(sf37_kvstore *kc, int tokens) {
    if (sf37_kvstore_continued_store_target(kc, tokens) != tokens) return -1;
    int old = kc->continued_last_store_tokens;
    sf37_kvstore_note_store(kc, tokens);
    return old;
}
...
static bool kv_cache_file_text_matches(const char *path, const char sha[41],
    ...
    char stored_sha[41];
    sf37_kvstore_sha1_bytes_hex(stored, text_bytes, stored_sha);
...
static bool kv_cache_existing_compatible(sf37_kvstore *kc, const char *path,
                                         const char sha[41],
                                         const char *text, size_t text_len,
                                         int model_id, int quant_bits, int ctx_size) {
    if (access(path, F_OK) != 0) return false;
    sf37_kvstore_entry e = {0};
    if (!sf37_kvstore_read_entry_file(path, sha, &e)) return false;
...
static bool kv_trailer_serialized_size(const sf37_kvstore_trailer_hooks *hooks,
                                       const char *text,
                                       uint64_t *bytes_out) {
...
static bool kv_trailer_write(const sf37_kvstore_trailer_hooks *hooks,
...
static void kv_cache_rewrite_trailer(sf37_kvstore *kc, const char *path,
                                     const char *text,
    ...
    uint32_t text_bytes = 0;
    bool ok = sf37_kvstore_read_header(fp, &hdr, &text_bytes);
    uint64_t end = SF37_KVSTORE_FIXED_HEADER + 4ull +
        ...
        ftruncate(fileno(fp), (off_t)end) == 0)
    {
        ...
        if (ok && ignored > 0) {
            uint8_t h[SF37_KVSTORE_FIXED_HEADER];
            uint64_t now = (uint64_t)time(NULL);
            sf37_kvstore_fill_header(h, hdr.model_id, hdr.quant_bits, hdr.reason,
...
bool sf37_kvstore_store_live_prefix_text(sf37_kvstore *kc,
    ...
    sf37_tokens store_tokens = {0};
    sf37_kvstore_tokens_copy_prefix(&store_tokens, tokens, store_len);

    (void)engine;
    const int quant_bits = SF37_KVSTORE_QUANT_BITS;
    const int model_id = SF37_KVSTORE_MODEL_ID;

    char save_err[160] = {0};
    const sf37_tokens *live_tokens = sf37_session_tokens(session);
    if (!live_tokens ||
        live_tokens->len != store_tokens.len ||
        !sf37_tokens_starts_with(live_tokens, &store_tokens))
    {
        kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
    ...
    if (text_override) {
        ...
        text = sf37_kvstore_render_tokens_text(engine, &store_tokens, &text_len);
    }
    ...
    char sha[41];
    sf37_kvstore_sha1_bytes_hex(text, text_len, sha);
    char *path = sf37_kvstore_path_for_sha(kc, sha);
    const uint8_t reason_code = sf37_kvstore_reason_code(reason);

    if (kv_cache_existing_compatible(kc, path, sha, text, text_len,
                                     model_id,
                                     quant_bits, sf37_session_ctx(session))) {
    ...
    sf37_session_payload_file staged = {0};
    if (sf37_session_stage_payload(session, &staged,
    ...
    if (!sf37_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                    trailer_est_bytes,
        ...
        sf37_session_payload_file_free(&staged);
        free(text);
        free(path);
        sf37_tokens_free(&store_tokens);
        return false;
    ...
    sf37_kvstore_eviction_context incoming = {
        .text = text,
        .text_len = text_len,
        .model_id = (uint8_t)model_id,
        .quant_bits = (uint8_t)quant_bits,
        .ctx_size = (uint32_t)sf37_session_ctx(session),
        .reject_different_quant = kc->reject_different_quant,
    };
    sf37_kvstore_evict(kc, live_tokens, est_file_bytes, &incoming);
    ...
    sf37_kvstore_fill_header(h, (uint8_t)model_id, (uint8_t)quant_bits,
                            reason_code, ext_flags,
                            (uint32_t)store_tokens.len, 0,
                            (uint32_t)sf37_session_ctx(session),
                            now, now, payload_bytes);
    uint8_t tb[4];
    sf37_kvstore_le_put32(tb, (uint32_t)text_len);
    uint64_t trailer_bytes = 0;
    errno = 0;
    bool ok = fwrite(h, 1, sizeof(h), fp) == sizeof(h) &&
              fwrite(tb, 1, sizeof(tb), fp) == sizeof(tb) &&
              fwrite(text, 1, text_len, fp) == text_len &&
              sf37_session_write_staged_payload(&staged, fp,
                                                save_err, sizeof(save_err)) == 0 &&
    ...
    if (ok && !sf37_kvstore_file_size_fits(kc, (uint64_t)text_len, payload_bytes,
                                          trailer_bytes,
                                          &final_file_bytes,
    ...
    if (!ok) {
        ...
        kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
                ...
                (double)(SF37_KVSTORE_FIXED_HEADER + 4ull + text_len + payload_bytes + trailer_bytes) / (1024.0 * 1024.0),
                save_ms);
    }
    sf37_session_payload_file_free(&staged);
    free(tmp);
...
bool sf37_kvstore_store_live_prefix(sf37_kvstore *kc,
                                   ...
                                   char *err,
                                   size_t err_len) {
    return sf37_kvstore_store_live_prefix_text(kc, engine, session, tokens,
...
bool sf37_kvstore_maybe_store_continued(sf37_kvstore *kc,
                                       ...
                                       char *err,
                                       size_t err_len) {
    const sf37_tokens *tokens = sf37_session_tokens(session);
    if (!tokens) return false;
    const int target = sf37_kvstore_continued_store_target(kc, tokens->len);
    if (target == 0) return false;
    if (sf37_kvstore_store_live_prefix(kc, engine, session, tokens, target,
                                      "continued", hooks, err, err_len))
    {
        sf37_kvstore_note_store(kc, target);
        return true;
    }
    return false;
}
...
int sf37_kvstore_find_text_prefix(sf37_kvstore *kc, const char *prompt_text,
    ...
    for (int i = 0; i < kc->len; i++) {
        sf37_kvstore_entry *e = &kc->entry[i];
        ...
        if (best >= 0) {
            sf37_kvstore_entry *b = &kc->entry[best];
            if (e->text_bytes < b->text_bytes) continue;
            if (e->text_bytes == b->text_bytes && e->tokens <= b->tokens) continue;
        }
        char sha[41];
        sf37_kvstore_sha1_bytes_hex(prompt_text, (size_t)e->text_bytes, sha);
...
int sf37_kvstore_try_load_text(sf37_kvstore *kc,
    ...
    const int model_id = SF37_KVSTORE_MODEL_ID;
    const size_t prompt_bytes = strlen(prompt_text);
    int idx = sf37_kvstore_find_text_prefix(kc, prompt_text, model_id, quant_bits,
                                           sf37_session_ctx(session));
    if (idx < 0) return 0;

    sf37_kvstore_entry e = kc->entry[idx];
    ...
    bool header_ok = sf37_kvstore_read_header(fp, &hdr, &text_bytes);
    ...
    if (header_ok) {
        if (hdr.model_id != (uint8_t)model_id) {
            ...
            if (fread(cached_text, 1, text_bytes, fp) != text_bytes) {
                ...
                char text_sha[41];
                sf37_kvstore_sha1_bytes_hex(cached_text, text_bytes, text_sha);
                if (strcmp(text_sha, e.sha)) {
                    header_ok = false;
                    fail_reason = "cached text hash mismatch";
                } else if (!sf37_kvstore_byte_prefix_match(prompt_text, prompt_bytes,
    ...
    if (header_ok) {
        bool payload_loaded =
            sf37_session_load_payload(session, fp, hdr.payload_bytes,
                                      err, sizeof(err)) == 0;
        const sf37_tokens *loaded_tokens = payload_loaded ?
            sf37_session_tokens(session) : NULL;
        ...
        if (header_ok) sf37_session_rewind(session, 0);
        kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
    ...
    if (loaded > 0) {
        ...
        const char *key_kind = sf37_kvstore_key_kind(hdr.ext_flags);
        bool consumed = false;
        if (kc->opt.cold_max_tokens > 0 && loaded > kc->opt.cold_max_tokens) {
            ...
            sf37_kvstore_touch_file(path, hdr.hits + 1);
            kv_logf(kc, SF37_KVSTORE_LOG_KVCACHE,
