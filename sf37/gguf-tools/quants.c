...
/*
 * SF37 asymmetric Q3 block.
 *
 * We intentionally store this under the GGUF Q3_K type id and size so the
 * tensor table remains compact. The byte layout is SF37-specific and must be
 * decoded by the SF37 engine, not by a generic Q3_K reader:
 *
 *   bytes 0..7:    four f16 scales, one per 64-value group
 *   bytes 8..11:   four uint8 zero-points in [0, 7]
 *   bytes 12..107: four packed 3-bit payload groups, 24 bytes each
 *   bytes 108..109: zero padding, keeping the standard 110-byte Q3_K size
 */
static void ds4q_write_q3_k_block_asym_g64(const float *x, uint8_t *y) {
    enum { scale_off = 0, zp_off = 8, qs_off = 12, group = 64 };
    memset(y, 0, ds4q_type_traits[DS4Q_TYPE_Q3_K].type_size);

    for (int g = 0; g < 4; g++) {
        const float *gx = x + g * group;
        float lo = gx[0], hi = gx[0];
        ...
        if (hi != lo) {
            scale = (hi - lo) / 7.0f;
            zp = ds4q_nearest_int(-lo / scale);
            zp = DS4Q_MAX(0, DS4Q_MIN(7, zp));
        } else if (hi != 0.0f) {
            scale = fabsf(hi) / 7.0f;
            zp = hi > 0.0f ? 0 : 7;
        }

        const uint16_t hscale = ds4q_f32_to_f16(scale);
        memcpy(y + scale_off + g * 2, &hscale, sizeof(hscale));
        y[zp_off + g] = (uint8_t)zp;

        uint8_t *qout = y + qs_off + g * 24;
        int bit = 0;
        const float inv = scale != 0.0f ? 1.0f / scale : 0.0f;
        for (int i = 0; i < group; i++) {
            int q = ds4q_nearest_int(gx[i] * inv + (float)zp);
            q = DS4Q_MAX(0, DS4Q_MIN(7, q));
            const int byte = bit >> 3;
            const int shift = bit & 7;
            qout[byte] |= (uint8_t)((q << shift) & 0xff);
            if (shift > 5) qout[byte + 1] |= (uint8_t)(q >> (8 - shift));
            bit += 3;
        }
    }
}

static int ds4q_pick_weighted_asym_group(const float *x, const float *w,
                                         int n, int qmax,
    ...
    for (int i = 0; i < n; i++) {
        if (x[i] < lo) lo = x[i];
        if (x[i] > hi) hi = x[i];
        const float wi = w && isfinite(w[i]) && w[i] > 0.0f ? w[i] : 0.0f;
        wsum += wi;
    }
    if (!w || wsum <= DS4Q_GROUP_MAX_EPS) return 0;
    ...
    if (hi != lo) {
        init_scale = (hi - lo) / (float)qmax;
    } else if (hi != 0.0f) {
        init_scale = fabsf(hi) / (float)qmax;
    }
    if (!(init_scale > 0.0f) || !isfinite(init_scale)) init_scale = 1.0f;

    for (int zp = 0; zp <= qmax; zp++) {
        ...
        for (int iter = 0; iter < 4; iter++) {
            const float inv = scale != 0.0f ? 1.0f / scale : 0.0f;
            for (int i = 0; i < n; i++) {
                int q = ds4q_nearest_int(x[i] * inv + (float)zp);
                q = DS4Q_MAX(0, DS4Q_MIN(qmax, q));
                qtmp[i] = (uint8_t)q;
            }
            ...
            double denom = 0.0;
            for (int i = 0; i < n; i++) {
                const float wi = w[i] > 0.0f && isfinite(w[i]) ? w[i] : 0.0f;
                const float a = (float)((int)qtmp[i] - zp);
                numer += (double)wi * (double)x[i] * (double)a;
                denom += (double)wi * (double)a * (double)a;
            }
            if (denom > (double)DS4Q_GROUP_MAX_EPS) {
                const float next = (float)(numer / denom);
                if (next > 0.0f && isfinite(next)) scale = next;
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            const float wi = w[i] > 0.0f && isfinite(w[i]) ? w[i] : 0.0f;
            const float deq = ((float)((int)qtmp[i] - zp)) * scale;
            const float d = x[i] - deq;
            err += (double)wi * (double)d * (double)d;
        }
        if (err < best_err) {
            best_err = (float)err;
            best_scale = scale;
            best_zp = zp;
            memcpy(best_q, qtmp, (size_t)n);
        }
    }

    if (!isfinite(best_err) || !(best_scale > 0.0f)) return 0;
    *scale_out = best_scale;
    *zp_out = best_zp;
    memcpy(q_out, best_q, (size_t)n);
    return 1;
}

static void ds4q_write_q3_k_block_asym_g64_weighted(const float *x, uint8_t *y,
                                                    const float *imatrix) {
    enum { scale_off = 0, zp_off = 8, qs_off = 12, group = 64 };
    if (!imatrix) {
        ds4q_write_q3_k_block_asym_g64(x, y);
        return;
    }
    memset(y, 0, ds4q_type_traits[DS4Q_TYPE_Q3_K].type_size);
    ...
    for (int g = 0; g < 4; g++) {
        const float *gx = x + g * group;
        const float *gw = imatrix + g * group;
        float scale = 1.0f;
        int zp = 0;
        uint8_t q[64];
        if (!ds4q_pick_weighted_asym_group(gx, gw, group, 7, &scale, &zp, q)) {
            uint8_t tmp[110];
            ds4q_write_q3_k_block_asym_g64(x, tmp);
            memcpy(y + scale_off + g * 2, tmp + scale_off + g * 2, 2);
            y[zp_off + g] = tmp[zp_off + g];
            memcpy(y + qs_off + g * 24, tmp + qs_off + g * 24, 24);
            continue;
        }
        ...
        for (int i = 0; i < group; i++) {
            const int byte = bit >> 3;
            const int shift = bit & 7;
            qout[byte] |= (uint8_t)(((int)q[i] << shift) & 0xff);
            if (shift > 5) qout[byte + 1] |= (uint8_t)((int)q[i] >> (8 - shift));
            bit += 3;
        }
    }
}
...
static size_t ds4q_quantize_q3_k(const float *src, void *dst, int64_t start,
    ...
    for (int64_t row = 0; row < nrows; row++) {
        ...
        for (int64_t b = 0; b < blocks_per_row; b++) {
            uint8_t *block = out + (size_t)row * row_size + (size_t)b * ds4q_type_traits[DS4Q_TYPE_Q3_K].type_size;
            if (imatrix) {
                ds4q_write_q3_k_block_asym_g64_weighted(xrow + (size_t)b * QK_K,
                                                        block,
                                                        imatrix + (size_t)b * QK_K);
            } else {
                ds4q_write_q3_k_block_asym_g64(xrow + (size_t)b * QK_K, block);
            }
        }
    }
    return (size_t)nrows * row_size;
...
/*
 * SF37 asymmetric Q2 block for routed down projections.
 *
 * Stored under the GGUF Q2_K type id and 84-byte block size:
 *
 *   bytes 0..7:    four f16 scales, one per 64-value group
 *   bytes 8..11:   four uint8 zero-points in [0, 3]
 *   bytes 12..75:  four packed 2-bit payload groups, 16 bytes each
 *   bytes 76..83:  zero padding
 */
static void ds4q_write_q2_k_block_asym_g64(const float *x, uint8_t *y) {
    enum { scale_off = 0, zp_off = 8, qs_off = 12, group = 64 };
    memset(y, 0, ds4q_type_traits[DS4Q_TYPE_Q2_K].type_size);
    ...
    for (int g = 0; g < 4; g++) {
        ...
        uint8_t *qout = y + qs_off + g * 16;
        const float inv = scale != 0.0f ? 1.0f / scale : 0.0f;
        for (int i = 0; i < group; i++) {
            int q = ds4q_nearest_int(gx[i] * inv + (float)zp);
            q = DS4Q_MAX(0, DS4Q_MIN(3, q));
            qout[i >> 2] |= (uint8_t)(q << ((i & 3) * 2));
        }
    }
}

static void ds4q_write_q2_k_block_asym_g64_weighted(const float *x, uint8_t *y,
                                                    const float *imatrix) {
    enum { scale_off = 0, zp_off = 8, qs_off = 12, group = 64 };
    if (!imatrix) {
        ds4q_write_q2_k_block_asym_g64(x, y);
        return;
    }
    memset(y, 0, ds4q_type_traits[DS4Q_TYPE_Q2_K].type_size);
    ...
    for (int g = 0; g < 4; g++) {
        ...
        int zp = 0;
        uint8_t q[64];
        if (!ds4q_pick_weighted_asym_group(gx, gw, group, 3, &scale, &zp, q)) {
            uint8_t tmp[84];
            ds4q_write_q2_k_block_asym_g64(x, tmp);
            memcpy(y + scale_off + g * 2, tmp + scale_off + g * 2, 2);
            y[zp_off + g] = tmp[zp_off + g];
            memcpy(y + qs_off + g * 16, tmp + qs_off + g * 16, 16);
            continue;
        }
        ...
        uint8_t *qout = y + qs_off + g * 16;
        for (int i = 0; i < group; i++) {
            qout[i >> 2] |= (uint8_t)(((int)q[i] & 3) << ((i & 3) * 2));
        }
    }
}

static size_t ds4q_quantize_q2_k_asym(const float *src, void *dst, int64_t start,
                                      int64_t nrows, int64_t ncols,
                                      const float *imatrix) {
    ...
    for (int64_t row = 0; row < nrows; row++) {
        ...
        for (int64_t b = 0; b < blocks_per_row; b++) {
            ...
            if (imatrix) {
                ds4q_write_q2_k_block_asym_g64_weighted(xrow + (size_t)b * QK_K,
                                                        block,
                                                        imatrix + (size_t)b * QK_K);
            } else {
                ds4q_write_q2_k_block_asym_g64(xrow + (size_t)b * QK_K, block);
            }
        }
    }
    return (size_t)nrows * row_size;
...
size_t ds4q_quantize_chunk(ds4q_type type, const float *src, void *dst,
    ...
    if (type == DS4Q_TYPE_Q2_K) {
        return ds4q_quantize_q2_k_asym(src, dst, start, nrows, ncols, imatrix);
    }
